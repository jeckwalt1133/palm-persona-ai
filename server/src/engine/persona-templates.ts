...
export const PERSONA_TEMPLATES: PersonaTemplate[] = [
  {
    ...
    weeklyAdvice: '这周试着把脑子里那个想法写下来——不用完美，写下来就行。',
  },
  {
    ...
    insightPool: ['你的忠诚是最珍贵的品质', '你懂得照顾别人——但也别忘了自己在那张名单上', '被认为无趣只是表面，懂你的人知道你多有趣', '慢热但长情，这是你的人际关系法则', '行动比语言更有力量，这是你的信条', '你的存在本身就是一种安心的信号'],
    keywordPool: ['可靠', '沉稳', '忠诚', '务实', '内敛'],
    weeklyAdvice: '这周试着对在乎的人多说一句"我也是"——他们需要听到。',
  },
  {
    type: 'flame-explorer',
    label: '火焰探索者',
    ...
    weeklyAdvice: '这周试着把一件事做到结束，而不只是开始。从最小的那件做起。',
  },
  {
    type: 'deep-thinker',
    label: '深海思考者',
    ...
    weeklyAdvice: '这周试着做一个"不用分析"的决定——就跟直觉走一次。',
  },
  {
    type: 'gentle-healer',
    label: '温柔疗愈者',
    ...
    insightPool: ['你的共情是天赐的礼物', '照顾好自己，再去照顾世界', '善良是选择，不是义务。你选择善良——也可以选择先保护自己', '你的存在就是一种安慰', '你的善良需要带一点锋芒', '别人愿意向你倾诉，因为你真的在听'],
    keywordPool: ['共情', '温暖', '治愈', '倾听', '包容'],
    weeklyAdvice: '这周试着先说"我需要"而不是"你需要什么"。顺序很重要。',
  },
  {
    type: 'sharp-pioneer',
    label: '锐意开拓者',
    ...
    insightPool: ['行动力是你的超能力', '偶尔慢下来，看到的风景同样重要', '你的果断是稀缺品质', '你跑得比别人快——偶尔回头看一眼，队伍跟上来了吗', '你的效率是团队里最稀缺的资源——用对地方比用得快更重要', '世界需要推动者，但也需要思考者——你可以兼顾'],
    keywordPool: ['果断', '执行力', '领导力', '效率', '进取'],
    weeklyAdvice: '这周试着留10分钟什么也不做——不计划、不推进、不等结果。',
  },
  {
    type: 'moon-artist',
    label: '月光艺术家',
    ...
    weeklyAdvice: '这周试着把一个作品发给一个人，不问评价，只发。',
  },
  {
    type: 'bridge-builder',
    label: '桥梁搭建者',
    ...
    insightPool: ['你的社交能力背后是真实的共情', '你是最好的调解人——但不是所有人的关系都该你负责', '你的朋友圈因为你而更紧密', '连接他人之前，先连接自己', '你让不同的声音找到了共同的频率', '真诚是最好的社交策略，你一直在用'],
    keywordPool: ['社交', '协调', '包容', '真诚', '凝聚力'],
    weeklyAdvice: '这周试着不主动调解一次——看看事情自己能不能好。',
  },
  {
    type: 'quiet-mountain',
    label: '静默山峰',
    ...
    insightPool: ['沉稳是你的底色，不是冷漠', '你心里的话有分量——说出来的时候，会有人认真听', '你的耐心是许多人最羡慕的品质', '你不需要改变自己来取悦任何人', '你的存在本身就是一种稳定的力量', '你的内心比表面看起来丰富得多'],
    keywordPool: ['内敛', '坚韧', '沉稳', '自洽', '深度'],
    weeklyAdvice: '这周试着多说一句——就一句。你会发现那句话本来就在嘴边。',
  },
  {
    type: 'sunshine-spark',
    label: '阳光火花',
    ...
    insightPool: ['你的乐观不是天真，是勇敢', '阴天也是天气的一种。你不发光的时候，依然是同一个人', '你散发的光和热，正在悄悄改变一些人', '保持这份明亮，世界需要你', '你的笑容背后是经历过风雨后的选择', '你让身边人的世界变得更轻了一些'],
    keywordPool: ['乐观', '阳光', '感染力', '热情', '轻盈'],
    weeklyAdvice: '这周试着让某个人看到你没在笑的样子。你不需要永远明亮。',
  },
  {
    type: 'wind-wanderer',
    label: '追风行者',
    ...
    insightPool: ['自由是你最珍贵的财富', '自由和方向不是敌人——画一张粗糙的地图，你可以走得更远', '你的随性让你拥有了别人计划不来的精彩', '风不需要解释为什么吹向某个方向', '你的生活方式是很多人心里的理想', '你不被框住的天性让你看到了更多可能'],
    keywordPool: ['自由', '随性', '浪漫', '旅人', '轻盈'],
    weeklyAdvice: '这周试着在一个地方多待半小时——看看有没有新发现。',
  },
  {
    type: 'root-keeper',
    label: '根系守护者',
    ...
    weeklyAdvice: '这周试着做一件计划外的事——不需要准备，想到就做。',
  },
];
...
export const CELEBRITY_MAP: Record<string, CelebrityEntry[]> = {
  ...
  'deep-thinker': [
    ...
    { name: 'Christopher Nolan', title: '导演', reason: '他的电影需要看三遍才懂——深度不是缺点，是门槛' },
    { name: '李安', title: '导演', reason: '每个镜头都在想"还能怎样"——深度是你和他共同的母语' },
  ],
  'gentle-healer': [
  ...
  'sharp-pioneer': [
    { name: 'Steve Jobs', title: 'Apple 创始人', reason: '现实扭曲力场——你的"先做了再说"和他"相信可以改变现实"是同一种信念' },
    { name: '谷爱凌', title: '运动员', reason: '做就做到最好——你的执行力是她那种大赛型选手级别的' },
    { name: '董明珠', title: '企业家', reason: '干脆利落——你的果断和她的风格一样：想清楚了就做' },
  ],
  'moon-artist': [
    { name: 'Billie Eilish', title: '歌手', reason: '不按流行套路出牌但全世界都在听——你的独特审美终将被看见' },
    { name: '王家卫', title: '导演', reason: '一个镜头能拍三个月——对作品有自己不愿妥协的标准，这一点你们很像' },
  ...
  'quiet-mountain': [
    ...
    { name: '北野武', title: '导演/演员', reason: '表面严肃，作品里全是温度和智慧——你也是外冷内热的类型' },
  ...
  'sunshine-spark': [
    ...
    { name: 'Robin Williams', title: '演员', reason: '让所有人笑的人，自己也需要一个可以不用笑的角落——你的热度下面有一颗柔软的心' },
  ],
  'wind-wanderer': [
    { name: '三毛', title: '作家', reason: '一辈子在走、在看、在写——她的自由里有一种你也会懂的诗意' },
...
export function generateRelationshipCode(scores: Array<{ dimensionKey: string; score: number }>): RelationshipCode {
  ...
  if (friction >= 65) {
    tensionPoint = '你在关系里最让人意外的地方是：你看起来很好说话，但有自己绝不会让的线。对方可能一开始没注意到——直到某天触到那条线才发现。这不是问题，是你的保护机制。';
  } else if (friction <= 35 && emo >= 65) {
    tensionPoint = '你在关系里藏得最深的地方是：忍。你以为忍过去就好了，但身体替你记住了每一次。试着在"算了"之前说一句"我在意"——不用大声，对方能听到就行。';
  } else if (trust <= 35 && comm <= 35) {
    tensionPoint = '你在关系里最让人难懂的地方是：你的信号不太容易被接收到。你的沉默有力量，但不等于所有人都能读懂它。偶尔多开一扇窗——不是让你改变自己，是让值得的人能走近一点。';
  } else {
    tensionPoint = '你在关系里最大的挑战是：有时候太在意自己的感受了——不是自私，是你太想知道"我在这段关系里是什么位置"。放轻松，最好的关系不是两个完全一样的人，是愿意为对方调一下频率的人。';
  }
...
export function assembleReport(
  ...
  return {
    ...
    weeklyAdvice: weeklyAdvice ?? template.weeklyAdvice,
    visualAnchors,
    identityBadge,
    adTeaser,
    relationshipCode,
